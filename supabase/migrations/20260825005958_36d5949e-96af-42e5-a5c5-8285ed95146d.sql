ALTER TABLE public.tasks ADD COLUMN IF NOT EXISTS days smallint[] NOT NULL DEFAULT '{}';
UPDATE public.tasks SET days = ARRAY[day_of_week]::smallint[] WHERE day_of_week IS NOT NULL AND days = '{}';
ALTER TABLE public.tasks ADD CONSTRAINT tasks_days_chk CHECK (
  COALESCE(array_length(days, 1), 0) <= 7
